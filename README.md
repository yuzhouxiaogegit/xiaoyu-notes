# 📝 小宇笔记

> 安全加密笔记管理系统，基于 Cloudflare 全栈技术构建

[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)
[![Cloudflare Pages](https://img.shields.io/badge/Cloudflare-Pages-orange)](https://pages.cloudflare.com/)
[![PWA Ready](https://img.shields.io/badge/PWA-Ready-green)](https://web.dev/progressive-web-apps/)

## ✨ 特性

### 🔒 安全加密

- **服务端加密存储**：所有笔记内容在服务端加密存储
- **分享时用户自定义加密**：分享链接支持用户设置独立密码
- **数据传输混淆**：API 传输过程中使用动态混淆算法
- **会话安全**：登录状态和管理密钥安全管理

### 🏷️ 分类管理

- **自定义分类**：支持创建、编辑、删除分类
- **智能统计**：实时显示每个分类下的笔记数量
- **分类筛选**：快速按分类浏览笔记

### 🔥 阅后即焚

- **访问次数限制**：支持设置 1次、3次、5次、10次或自定义次数
- **自动销毁**：达到访问限制后自动删除分享内容
- **密码保护**：分享链接支持独立密码保护

### 📱 现代化体验

- **PWA 支持**：可安装到桌面，支持离线使用
- **响应式设计**：完美适配手机、平板、桌面设备
- **暗色主题**：护眼的深色界面设计
- **实时字数统计**：写作时实时显示字数和进度

## 🚀 快速开始

### 环境要求

- Node.js 18+
- Cloudflare 账户
- Wrangler CLI

### 本地开发

1. **克隆项目**

   ```bash
   git clone https://github.com/yuzhouxiaogegit/xiaoyu-notes.git
   cd xiaoyu-notes
   ```

2. **安装依赖**

   ```bash
   npm install
   ```

3. **配置环境变量**

   创建 `.dev.vars` 文件：

   ```bash
   # 登录凭据
   LOGIN_USERNAME=admin
   LOGIN_PASSWORD=your-secure-password

   # 管理密钥
   ADMIN_KEY=your-admin-key
   ```

4. **启动开发服务器**

   ```bash
   npm run dev
   ```

5. **访问应用**

   打开 http://localhost:8788

### 部署到 Cloudflare Pages

1. **创建 D1 数据库**

   ```bash
   wrangler d1 create xiaoyu-notes-db
   ```

2. **更新 wrangler.toml**

   将生成的数据库 ID 填入 `wrangler.toml`：

   ```toml
   [[d1_databases]]
   binding = "DB"
   database_name = "xiaoyu-notes-db"
   database_id = "your-database-id"
   ```

3. **执行数据库迁移**

   ```bash
   wrangler d1 execute xiaoyu-notes-db --file=./schema.sql
   ```

4. **部署应用**

   ```bash
   npm run deploy
   ```

5. **配置环境变量**

   在 Cloudflare Pages 控制台中设置：
   - `LOGIN_USERNAME`: 登录用户名
   - `LOGIN_PASSWORD`: 登录密码
   - `ADMIN_KEY`: 管理员密钥

## 🏗️ 技术架构

### 前端技术栈

- **原生 JavaScript**：无框架依赖，轻量高效
- **Tailwind CSS CDN**：现代化 UI 设计
- **PWA**：支持离线使用和桌面安装
- **模块化架构**：代码分离，按需加载

### 后端技术栈

- **Cloudflare Pages Functions**：无服务器函数
- **Cloudflare D1**：SQLite 数据库
- **Cloudflare Workers AI**：AI 功能支持
- **数据混淆**：自定义传输安全机制

### 安全机制

- **AES-GCM 256-bit 加密**：客户端加密算法
- **PBKDF2 密钥派生**：安全的密钥生成
- **动态数据混淆**：传输过程安全保护
- **时间窗口验证**：防止重放攻击

## 📁 项目结构

```
xiaoyu-notes/
├── public/                 # 前端静态文件
│   ├── assets/
│   │   ├── css/           # 样式文件
│   │   └── js/            # JavaScript 模块
│   │       ├── core/      # 核心框架
│   │       ├── api.js     # API 接口
│   │       ├── auth.js    # 认证模块
│   │       ├── notes.js   # 笔记管理
│   │       ├── share.js   # 分享功能
│   │       └── ...        # 其他模块
│   ├── index.html         # 主页面
│   ├── manifest.json      # PWA 配置
│   └── sw.js             # Service Worker
├── functions/             # 后端函数
│   ├── api/              # API 处理器
│   ├── core/             # 核心基类
│   ├── handlers/         # 业务处理器
│   ├── utils/            # 工具函数
│   └── _middleware.js    # 中间件
├── wrangler.toml         # Cloudflare 配置
├── package.json          # 项目配置
└── README.md            # 项目文档
```

## 🔧 配置说明

### 环境变量

| 变量名               | 说明           | 示例值                    |
| -------------------- | -------------- | ------------------------- |
| `LOGIN_USERNAME`     | 登录用户名     | `admin`                   |
| `LOGIN_PASSWORD`     | 登录密码       | `your-secure-password`    |
| `ADMIN_KEY`          | 管理员密钥     | `your-admin-key-here`     |
| `SERVER_ENCRYPT_KEY` | 服务端加密密钥 | `your-server-encrypt-key` |

### 数据库配置

项目使用 Cloudflare D1 数据库，表结构包括：

- `notes` - 笔记内容表
- `categories` - 分类管理表
- `shares` - 分享链接表

## 🛡️ 安全特性

### 数据加密

- 笔记内容在服务端使用服务器密钥加密存储
- 分享功能支持用户自定义密码的额外加密层
- 所有敏感数据传输过程中进行动态混淆

### 访问控制

- 基于会话的身份验证
- 管理员密钥验证
- 防重放攻击机制

### 隐私保护

- 本地开发环境使用模拟数据
- 生产环境数据完全隔离
- 支持数据完全删除

## 📱 PWA 功能

- **离线支持**：缓存关键资源，支持离线浏览
- **桌面安装**：可安装为桌面应用
- **推送通知**：支持重要提醒（可选）
- **自动更新**：Service Worker 自动更新机制

## 🤝 贡献指南

1. Fork 本仓库
2. 创建特性分支 (`git checkout -b feature/AmazingFeature`)
3. 提交更改 (`git commit -m 'Add some AmazingFeature'`)
4. 推送到分支 (`git push origin feature/AmazingFeature`)
5. 开启 Pull Request

## 📄 许可证

本项目采用 MIT 许可证 - 查看 [LICENSE](LICENSE) 文件了解详情

## 👤 作者

**宇宙小哥**

- GitHub: [@yuzhouxiaogegit](https://github.com/yuzhouxiaogegit)
- 项目链接: [https://github.com/yuzhouxiaogegit/xiaoyu-notes](https://github.com/yuzhouxiaogegit/xiaoyu-notes)

## 🙏 致谢

- [Cloudflare](https://cloudflare.com/) - 提供强大的边缘计算平台
- [Tailwind CSS](https://tailwindcss.com/) - 现代化的 CSS 框架
- 所有贡献者和用户的支持

---

⭐ 如果这个项目对你有帮助，请给它一个星标！
